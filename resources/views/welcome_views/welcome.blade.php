<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://unpkg.com/tailwindcss@^1.0/dist/tailwind.min.css" rel="stylesheet">
    <link rel="icon" type="image/png" href="images/logo.png">
    <title>My University Progress</title>
</head>
<body class="bg-purple-500 text-white" style="background-image: url(/images/login/bg_login.png); background-size:cover;">
<body class=" text-white">
    <div class="py-5 px-5 text-center">
            <a href="" class="hover:text-white-200 font-bold text-lg lg:text-6xl">
                My University Progress
            </a>
    </div>
    

    <div class="grid grid-cols-1 lg:grid-cols-2 container mx-auto my-10 items-center text-center">
        <div class="px-2">
            <img src="images/logo_white.png" alt="Logo" class="w-auto px-2">
        </div>
        <div >
            <a href="about/">
                <button class="bg-white hover:bg-purple-100 text-purple-400 font-bold py-2 px-4 rounded margin-white">
                    Acerca de nosotros
                </button>
            </a>
            <br><br>
            <a href="{{route('login')}}">
                <button class="bg-white hover:bg-purple-100 text-purple-400 font-bold py-2 px-4 rounded margin-white">
                    Login!
                </button>
            </a>
            <div class="mx-auto w-1/2 mt-2">
                <p class="italic my-1">Universidades:</p>
                @foreach ($universities as $university)
                    <img src="{{asset("storage/Universities/{$university->logo}")}}" alt="{{$university->name}}" class="w-10 mx-auto rounded">
                @endforeach
            </div>
        </div>
    </div>
</body>
</html